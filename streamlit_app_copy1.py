...
def predict_category(headline, model, tokenizer, label_encoder):
    """
    Predict the category for a given headline, timing only the model's computation.
    ...
    Returns:
        tuple: Predicted category label and time taken for the model's computation (in seconds).
    """
    # Preprocessing
    inputs = tokenizer(
        headline, 
    ...
    # Timing only the model's forward pass
    start_time = time.time()  # Start timer for the model
    with torch.no_grad():
    ...
    elapsed_time = end_time - start_time  # Calculate elapsed time for model prediction
    return predicted_label[0], elapsed_time
...
if st.button("Classify"):
    ...
    if headline.strip():
        ...
        with st.spinner("Classifying the headline..."):
            category, elapsed_time = predict_category(headline, model, tokenizer, label_encoder)
        # Display the predicted category and the time taken
        st.markdown(
            f"<div class='result-box'>"
            f"Predicted Category: <b>{category}</b><br>"
            f"Time Taken by Model: <b>{elapsed_time:.2f} seconds</b>"
            f"</div>",
            unsafe_allow_html=True
        )
    else:
